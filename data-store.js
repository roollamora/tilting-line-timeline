  ...
  function sanitize(data) {
    ...
    return {
      timeline: {
        ...
        perspectiveDepth: Number.isFinite(Number(timeline.perspectiveDepth)) ? Math.max(1, Math.min(3, Number(timeline.perspectiveDepth))) : fallback.timeline.perspectiveDepth
      },
      angleNodes: (angleNodes.length ? angleNodes : fallback.angleNodes).map((node, index) => ({
